...
function getQueryAndUrl (state, options, apiDefinitions, format) {
  ...
  var path = apiDefinitions.path

  if (state.pathPrefix && path.indexOf(state.pathPrefix) !== 0) {
    path = state.pathPrefix + apiDefinitions.path
  }
  ...
  Object.keys(apiDefinitions.params).forEach(function (paramName) {
    ...
    var isUrlParam = path.indexOf(':' + paramName) !== -1
