...
(function() {
    ...
    /**
     * Get an OAuth access token for a user for use when authenticating
     * next requests using an API token
     *
     * @param {String} code           Code posted to you by github
     * @param {String} clientId       GitHub application client id
     * @param {String} clientSecret   GitHub application client secret
     * @return {Array}                data returned
     */
    this.getOAuthAccessToken = function(code, clientId, clientSecret, callback)
    {
        var reqOptions = {
        ...
        return this.post('login/oauth/access_token', postData, reqOptions, function processOAuthTokenResponse(err, response) {
            // Since the request is cached we need to reset the options
            // Maybe looking these up in the $defaults is preferable?
            self.getRequest()
                .setOption('path', '/api/v2')
                .setOption('format', 'json');

            if (err) {
                callback(err, null);
            }

            response = require('querystring').parse(response);
            if (response.error) {
