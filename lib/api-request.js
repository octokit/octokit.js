...
function apiRequest (state, options, apiDefinitions, callback) {
  ...
  var protocol = state.protocol || state.definitions.constants.protocol
  var port = state.port || (protocol === 'https' ? 443 : 80)
  var host = state.host || state.definitions.constants.host
  ...
  function addCustomHeaders (customHeaders) {
    Object.keys(customHeaders).forEach(function (header) {
      ...
      if (state.requestHeaders.indexOf(headerLC) === -1) {
        return
      }
  ...
  addCustomHeaders(Object.assign(options.headers || {}, state.headers))
  ...
  if (!('accept' in headers)) {
    headers['accept'] = state.previewHeaderByPath[apiDefinitions.url] || state.requestMedia || state.definitions.constants.requestMedia
  }
  ...
  httpRequest(requestOptions, (error, response) => {
    if (error) {
      return sendError(error, options, null, callback)
    }
    ...
    state.responseHeaders.forEach(function (header) {
      if (response.headers[header]) {
        result.meta[header] = response.headers[header]
      }
    })
