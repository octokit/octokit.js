...
endpoints.forEach(endpoint => {
  ...
  newRoutes[scope][idName] = {
    ...
    params: endpoint.params.reduce((result, param) => {
      ...
      if (param.mapTo) {
        result[param.name].mapTo =
          param.mapTo === "input" ? "data" : param.mapTo;
  ...
  if (endpoint.deprecated) {
    if (endpoint.deprecated.after) {
      newRoutes[scope][idName].deprecated = `octokit.${scope}.${camelCase(
        endpoint.deprecated.before.idName
      )}() has been renamed to octokit.${scope}.${camelCase(
        endpoint.deprecated.after.idName
