...
(function () {
	...
	this.httpSend = function (msg, block, callback) {
		...
		function httpSendRequest() {
			...
			_options =  JSON.parse(JSON.stringify(options)); // deep copy

			if (!self.config.etagHandler) {
				return doHttpSendRequest();
			}
			self.config.etagHandler.getETAG(_options, function (err, ETAG) {
			...
			var req = require(protocol).request(options, function (res) {
				...
				res.on("end", function () {
					if (res.statusCode >= 400 && res.statusCode < 600 || res.statusCode < 10) {
						...
						res.data = data;
						if (!self.config.etagHandler || !res.headers || !res.headers.etag) {
							return callCallback(null, res);
						}
						self.config.etagHandler.setETAG(_options, res.headers.etag, function (err) {
							if (err) throw new Error(err);
							return callCallback(null, res);
						});
					}
